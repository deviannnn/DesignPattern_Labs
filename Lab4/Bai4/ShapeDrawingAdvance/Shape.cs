...
namespace ShapeDrawingAdvance
{
    public abstract class Shape
    {
        ...
        protected ILineBehavior LineBehavior = new SolidLine();
        protected IPaintBehavior PaintBehavior = new EmptyPaint();

        public abstract void Draw(Graphics g);
        public abstract bool Include(Point p);
        public void SetLineBehavior(ILineBehavior lineBehavior)
        {
            this.LineBehavior = lineBehavior;
        }

        public void SetPaintBehavior(IPaintBehavior paintBehavior)
        {
    ...
    public abstract class Triangle : Shape
    {
        public sealed override void Draw(Graphics g)
        {
            ...
            using (Pen pen = LineBehavior.GetPenLine(Color))
            {
                g.DrawPolygon(pen, new Point[] { p1, p2, p3 });
            }
            using (Brush? brushPaint = PaintBehavior.GetBrushPaint())
            {
                if (brushPaint != null)
                {
                    g.FillPolygon(brushPaint, new Point[] { p1, p2, p3 });
                }
            }
        }
    ...
    public class Circle : Shape
    {
        ...
        public override void Draw(Graphics g)
        {
            ...
            using (Brush? brushPaint = PaintBehavior.GetBrushPaint())
            {
                if (brushPaint != null)
                {
                    g.FillEllipse(brushPaint, GetRectangle());
                }
            }
        }
    ...
    public class Square : Shape
    {
        ...
        public override void Draw(Graphics g)
        {
            ...
            using (Brush? brushPaint = PaintBehavior.GetBrushPaint())
            {
                if (brushPaint != null)
                {
                    g.FillRectangle(brushPaint, GetRectangle());
                }
            }
        }
